...
/**
 * @fileOverview A flow for correcting common OCR mistakes in mathematical equations.
 *
...
const CorrectEquationMistakesInputSchema = z.object({
  ocrText: z.string().describe('The OCR output of a mathematical equation.'),
});
...
const correctEquationMistakesPrompt = ai.definePrompt({
  ...
  prompt: `You are an expert in correcting common OCR mistakes in math equations. Your goal is to produce a valid mathematical expression that can be solved.

You will receive the OCR output of an equation that could be handwritten or digitally typed. Your task is to correct common mistakes. This includes, but is not limited to:
